...
bool RedisClient::Connect(const char* host, int port){
  ...
  if (sock_fd == INVALID_SOCKET) {
    std::cerr << "Socket creation failed error: " << GET_SOCKET_ERROR() << "\n";
    return false;
  }
  ...
  if (connect(sock_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
    std::cerr << "Connection failed error: " << GET_SOCKET_ERROR() << "\n";
    CLOSE_SOCKET(sock_fd);
    sock_fd = INVALID_SOCKET;
    return false;
  }
...
bool RedisClient::CheckedReadResponse(){
  ...
  resp_parser.ParseBuffer(buffer, read);
  std::vector<RespObject> objects = resp_parser.GetObjects();

  resp_parser.PrintResp(objects[0]);
  return true;
}